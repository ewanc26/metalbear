...
void throw_errno_socket(const std::string &what) {
    ...
    if (errno == ECONNRESET)
        throw PyError("ConnectionResetError",
...
int connect_with_timeout(const std::string &host, int port,
    ...
    freeaddrinfo(res);
    if (fd < 0)
        throw PyError("OSError", std::string("connect to ") + host + ":" +
                                     std::to_string(port) +
    ...
    if (SSL_CTX_set_default_verify_paths(ctx) != 1) {
        ...
        if (!ok) {
            ...
            throw std::runtime_error(
                "cannot load CA certificates for TLS verification");
        }
    }
    return ctx;
}
...
void ssl_write_all(SSL *ssl, const std::string &data) {
    size_t off = 0;
    while (off < data.size()) {
        int n = SSL_write(ssl, data.data() + off,
...
class FrameReader {
  public:
    ...
    std::pair<int, std::vector<uint8_t>> frame() {
        ...
        if (ln > (1u << 30)) throw PyError("RuntimeError", "frame too large");
...
scan_cid_links(const std::vector<uint8_t> &payload) {
    int good = 0;
...
void scan_kinds(const std::vector<uint8_t> &payload,
                std::vector<std::string> &out) {
    static const char *kKinds[] = {"commit", "identity", "account",
                                   "sync",   "info",     "handle"};
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        if (!tls.ssl)
            throw std::runtime_error("SSL_new: " + ssl_error_string());
    ...
    try {
        ...
        size_t nl = head.find("\r\n");
        status = decode_replace(
            head.substr(0, nl == std::string::npos ? head.size() : nl));
    } catch (const std::exception &e) {
        std::cerr << "  ERROR: " << e.what() << "\n";
        return 1;
    }
    ...
    try {
        while (std::chrono::steady_clock::now() < deadline) {
            ...
            total_bad.insert(total_bad.end(), links.second.begin(),
                             links.second.end());
            if (frames <= 2)
                std::cout << "  frame " << frames << ": " << payload.size()
                          << " bytes, " << links.first << " CID links\n";
        }
    ...
    if (kinds.empty()) {
        ...
        for (size_t i = 0; i < kinds.entries.size(); i++) {
            if (i) std::cout << ", ";
            std::cout << "'" << kinds.entries[i].first
                      << "': " << kinds.entries[i].second;
        }
        std::cout << "}\n";
    }
    ...
    if (!total_bad.empty()) {
        std::cout << "  RESULT: FAIL — malformed CID links; a strict reader "
                     "rejects these frames\n";
        return 1;
    }
    if (total_good > 0) {
        std::cout
            << "  RESULT: PASS — every CID link carries the multibase prefix\n";
        return 0;
    }
    bool saw_ping = false;
    ...
    if (saw_ping) {
        std::cout << "  RESULT: stream alive (keepalive only, no writes during "
                     "the window)\n";
        return 0;
    }
