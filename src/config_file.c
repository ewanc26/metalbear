...
#define FAIL(line, fmt, ...)                                                   \
    do {                                                                       \
        if (err && err_len)                                                    \
            snprintf(err, err_len, "%s:%d: " fmt, path, (line),                \
                     ##__VA_ARGS__);                                           \
        goto fail;                                                             \
    } while (0)
    ...
    *out_owner = NULL;
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *eq = '\0';
        ...
        char full[128];
        snprintf(full, sizeof(full), "%s%s%s", section, *section ? "." : "",
                 key);
...
#define INT(name, field)                                                       \
    if (strcmp(full, name) == 0) {                                             \
        if (!parse_int(val, &iv))                                              \
            FAIL(lineno, "'%s' expects an integer", key);                      \
        config->field = iv;                                                    \
        continue;                                                              \
    }
...
#define BOOL(name, field)                                                      \
    if (strcmp(full, name) == 0) {                                             \
        if (!parse_bool(val, &bv))                                             \
            FAIL(lineno, "'%s' expects true or false", key);                   \
        config->field = bv;                                                    \
        continue;                                                              \
    }
...
#undef UINT
        ...
        if (strcmp(full, "firehose.crawlers") == 0) {
            ...
            if (!joined) {
                ...
                if (!joined)
                    FAIL(lineno, "'crawlers' expects an array or string");
            }
