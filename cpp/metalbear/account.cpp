...
static wf_status derive_password(const char *password,
    ...
    if (!password ||
        EVP_PBE_scrypt(password, std::strlen(password), salt, 16, 16384, 8, 1,
                       32 * 1024 * 1024, hash, 32) != 1)
...
wf_status metalbear_account_store_open(const char *path,
    ...
    if (!path || !bootstrap_password || !out) return WF_ERR_INVALID_ARG;
    *out = nullptr;
    auto *store = static_cast<metalbear_account_store *>(
        std::calloc(1, sizeof(metalbear_account_store)));
    ...
    const char *sql =
        ...
        "id INTEGER PRIMARY KEY CHECK(id=0),"
        "data TEXT NOT NULL);";
    if (sqlite3_exec(store->db.get(), sql, nullptr, nullptr, nullptr) !=
        SQLITE_OK) {
    ...
    sqlite3_exec(store->db.get(),
                 "ALTER TABLE account_state ADD COLUMN invites_enabled INTEGER "
                 "NOT NULL DEFAULT 1;",
                 nullptr, nullptr, &err);
    ...
    if (sqlite3_prepare_v2(store->db.get(),
                           "SELECT 1 FROM credentials WHERE id=0;", -1, &stmt,
                           nullptr) == SQLITE_OK &&
...
int metalbear_account_verify_password(metalbear_account_store *store,
    ...
    if (sqlite3_prepare_v2(
            store->db.get(),
            "SELECT salt,password_hash FROM credentials WHERE id=0;", -1, &stmt,
            nullptr) == SQLITE_OK &&
        sqlite3_step(stmt) == SQLITE_ROW) {
...
metalbear_account_verify_credential(metalbear_account_store *store,
    ...
    if (sqlite3_prepare_v2(
            store->db.get(),
            "SELECT name,salt,password_hash,privileged FROM app_password;", -1,
            &stmt, nullptr) == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            const char *name =
                reinterpret_cast<const char *>(sqlite3_column_text(stmt, 0));
            const unsigned char *salt = static_cast<const unsigned char *>(
                sqlite3_column_blob(stmt, 1));
            const unsigned char *expected = static_cast<const unsigned char *>(
...
wf_status metalbear_account_create_app_password(metalbear_account_store *store,
                                                ...
                                                char **out_password,
                                                char **out_created_at) {
    if (!store || !name || !name[0] || std::strlen(name) > 256 ||
        !out_password || !out_created_at)
        return WF_ERR_INVALID_ARG;
    ...
    *out_created_at = nullptr;
    ...
    if (derive_password(formatted, salt, hash) != WF_OK) return WF_ERR_INTERNAL;
    ...
    *out_count = 0;
    ...
    while (status == WF_OK && sqlite3_step(stmt) == SQLITE_ROW) {
        if (*out_count == capacity) {
            ...
            void *resized =
                std::realloc(*out_passwords, next * sizeof(**out_passwords));
            if (!resized) {
                status = WF_ERR_ALLOC;
                break;
            }
...
wf_status metalbear_account_revoke_app_password(metalbear_account_store *store,
    ...
    wf_status status = WF_ERR_INTERNAL;
    if (sqlite3_prepare_v2(store->db.get(),
                           "DELETE FROM app_password WHERE name=?;", -1, &stmt,
                           nullptr) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, name, -1, SQLITE_TRANSIENT);
...
int metalbear_account_is_active(metalbear_account_store *store) {
    ...
    int active = 0;
    if (sqlite3_prepare_v2(store->db.get(),
                           "SELECT active FROM account_state WHERE id=0;", -1,
                           &stmt, nullptr) == SQLITE_OK &&
        sqlite3_step(stmt) == SQLITE_ROW)
...
wf_status metalbear_account_get_email(metalbear_account_store *store,
    ...
    *out_email = nullptr;
    ...
    if (sqlite3_prepare_v2(
            store->db.get(),
            "SELECT email,email_confirmed FROM account_state WHERE id=0;", -1,
            &stmt, nullptr) == SQLITE_OK &&
        sqlite3_step(stmt) == SQLITE_ROW) {
        const char *email =
            reinterpret_cast<const char *>(sqlite3_column_text(stmt, 0));
...
wf_status metalbear_account_create_email_token(metalbear_account_store *store,
    ...
    if (RAND_bytes(random_bytes, sizeof(random_bytes)) != 1)
        return WF_ERR_CRYPTO;
    for (size_t i = 0; i < sizeof(random_bytes) && i * 2 + 1 < token_len; i++) {
        out_token[i * 2] = hex[random_bytes[i] >> 4];
        out_token[i * 2 + 1] = hex[random_bytes[i] & 15];
    }
    out_token[32] = '\0';
    char created_at[32];
    if (current_datetime(created_at) != WF_OK) return WF_ERR_INTERNAL;
    pthread_mutex_lock(&store->mutex);
    sqlite3_stmt *stmt = nullptr;
    wf_status status = WF_ERR_INTERNAL;
    if (sqlite3_prepare_v2(
...
wf_status metalbear_account_verify_email_token(metalbear_account_store *store,
    ...
    wf_status status = WF_ERR_INTERNAL;
    if (sqlite3_prepare_v2(store->db.get(),
                           "DELETE FROM email_token WHERE token=? AND kind=? "
                           "AND expires_at>?;",
                           -1, &stmt, nullptr) == SQLITE_OK) {
    ...
    if (status == WF_OK && std::strcmp(kind, "confirm") == 0) {
        ...
        if (sqlite3_prepare_v2(
                store->db.get(),
                "UPDATE account_state SET email_confirmed=1 WHERE id=0;", -1,
                &stmt2, nullptr) == SQLITE_OK) {
            sqlite3_step(stmt2);
        }
...
wf_status metalbear_account_delete(metalbear_account_store *store) {
    ...
    sqlite3_stmt *stmt = nullptr;
    wf_status status = WF_ERR_INTERNAL;
    if (sqlite3_prepare_v2(store->db.get(), "DELETE FROM app_password;", -1,
                           &stmt, nullptr) == SQLITE_OK) {
        sqlite3_step(stmt);
    }
    sqlite3_finalize(stmt);
    if (sqlite3_prepare_v2(store->db.get(), "DELETE FROM credentials;", -1,
                           &stmt, nullptr) == SQLITE_OK) {
        sqlite3_step(stmt);
    }
    sqlite3_finalize(stmt);
    if (sqlite3_prepare_v2(store->db.get(),
                           "UPDATE account_state SET active=0 WHERE id=0;", -1,
                           &stmt, nullptr) == SQLITE_OK) {
        if (sqlite3_step(stmt) == SQLITE_DONE) status = WF_OK;
    }
    sqlite3_finalize(stmt);
...
wf_status metalbear_account_reset_password(metalbear_account_store *store,
    ...
    if (sqlite3_prepare_v2(
            store->db.get(),
            "UPDATE credentials SET salt=?,password_hash=? WHERE id=0;", -1,
            &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_blob(stmt, 1, salt, sizeof(salt), SQLITE_TRANSIENT);
        sqlite3_bind_blob(stmt, 2, hash, sizeof(hash), SQLITE_TRANSIENT);
        sqlite3_step(stmt);
...
wf_status metalbear_account_store_prefs_get(metalbear_account_store *store,
    ...
    *out_json = nullptr;
    ...
    sqlite3_stmt *stmt = nullptr;
    if (sqlite3_prepare_v2(store->db.get(),
                           "SELECT data FROM preferences WHERE id=0;", -1,
                           &stmt, nullptr) != SQLITE_OK ||
        sqlite3_step(stmt) != SQLITE_ROW) {
...
bool metalbear_account_invites_enabled(metalbear_account_store *store) {
    ...
    if (sqlite3_prepare_v2(
            store->db.get(),
            "SELECT invites_enabled FROM account_state WHERE id=0;", -1, &stmt,
            nullptr) == SQLITE_OK &&
        sqlite3_step(stmt) == SQLITE_ROW) {
...
wf_status metalbear_account_set_invites_enabled(metalbear_account_store *store,
    ...
    if (sqlite3_prepare_v2(
            store->db.get(),
            "UPDATE account_state SET invites_enabled=? WHERE id=0;", -1, &stmt,
            nullptr) == SQLITE_OK) {
