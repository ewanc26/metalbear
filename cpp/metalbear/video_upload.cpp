...
void expire_sessions_locked(metalbear_video_upload_store *store) {
    auto query =
        prepare(store->db, "SELECT job_id FROM video_uploads WHERE state=0 AND "
...
bool load_status_locked(metalbear_video_upload_store *store, const char *job_id,
    ...
    auto stmt = prepare(
        ...
        "completed_job_id,cid,failure_reason FROM video_uploads WHERE "
        "job_id=?;");
    ...
    copy_text(
        out->completed_job_id, sizeof(out->completed_job_id),
        reinterpret_cast<const char *>(sqlite3_column_text(stmt.get(), 6)));
    copy_text(
        out->cid, sizeof(out->cid),
        reinterpret_cast<const char *>(sqlite3_column_text(stmt.get(), 7)));
    copy_text(
        out->failure_reason, sizeof(out->failure_reason),
        reinterpret_cast<const char *>(sqlite3_column_text(stmt.get(), 8)));

    auto parts =
        prepare(store->db, "SELECT part_number FROM video_upload_parts WHERE "
...
state_result(const metalbear_video_upload_status &status) {
    switch (status.state) {
...
void limits_locked(metalbear_video_upload_store *store, uint64_t *bytes,
    ...
    if (stmt && sqlite3_step(stmt.get()) == SQLITE_ROW) {
        used_bytes = static_cast<uint64_t>(sqlite3_column_int64(stmt.get(), 0));
...
metalbear_video_upload_result
assemble_upload(metalbear_video_upload_store *store,
                const metalbear_video_upload_status &status,
                std::string &assembled, std::string &cid_string) {
...
metalbear_video_upload_start(metalbear_video_upload_store *store,
    ...
    try {
        ...
        const uint32_t part_count =
            static_cast<uint32_t>((size_bytes + part_size - 1) / part_size);
        ...
        sqlite3_bind_int64(stmt.get(), 2,
                           static_cast<sqlite3_int64>(size_bytes));
        ...
        sqlite3_bind_int64(stmt.get(), 5,
                           static_cast<sqlite3_int64>(part_size));
        ...
        std::error_code error;
        fs::create_directories(fs::path(store->parts_directory) / job_id,
                               error);
        if (error) {
            auto remove =
                prepare(store->db, "DELETE FROM video_uploads WHERE job_id=?;");
            if (remove) {
...
metalbear_video_upload_part_begin(metalbear_video_upload_store *store,
    ...
    *out = nullptr;
    ...
    try {
        ...
        const uint64_t expected = part_number == status.part_count
...
metalbear_video_upload_part_finish(metalbear_video_part_writer *writer) {
    if (!writer || !writer->file) return METALBEAR_VIDEO_UPLOAD_INVALID_REQUEST;
    if (writer->written != writer->expected_size)
        return METALBEAR_VIDEO_UPLOAD_PART_SIZE_MISMATCH;
...
metalbear_video_upload_result metalbear_video_upload_finish(
    ...
    try {
        metalbear_video_upload_status status{};
        {
            ...
            auto update =
                prepare(store->db, "UPDATE video_uploads SET state=1 WHERE "
                                   "job_id=? AND state=0;");
        ...
        if (result == METALBEAR_VIDEO_UPLOAD_OK &&
            metalbear_blob_store_put_file(
                store->blobs, cid.c_str(), status.mime_type, assembled.c_str(),
                static_cast<size_t>(status.size_bytes)) != WF_OK)
            result = METALBEAR_VIDEO_UPLOAD_SERVICE_OVERLOADED;
        ...
        if (result == METALBEAR_VIDEO_UPLOAD_OK) {
            ...
            sqlite3_bind_text(update.get(), 1, completed.c_str(), -1,
                              SQLITE_TRANSIENT);
            sqlite3_bind_text(update.get(), 2, cid.c_str(), -1,
                              SQLITE_TRANSIENT);
            ...
            auto update =
                prepare(store->db, "UPDATE video_uploads SET state=0 WHERE "
                                   "job_id=? AND state=1;");
            if (update) {
                sqlite3_bind_text(update.get(), 1, job_id, -1,
                                  SQLITE_TRANSIENT);
...
metalbear_video_upload_abort(metalbear_video_upload_store *store,
    ...
    try {
        ...
        if (out->state == METALBEAR_VIDEO_UPLOAD_CREATED) {
            auto update =
                prepare(store->db, "UPDATE video_uploads SET state=4 WHERE "
                                   "job_id=? AND state=0;");
            if (!update) return METALBEAR_VIDEO_UPLOAD_INTERNAL;
            sqlite3_bind_text(update.get(), 1, job_id, -1, SQLITE_TRANSIENT);
            if (sqlite3_step(update.get()) != SQLITE_DONE)
            ...
            fs::remove_all(fs::path(store->parts_directory) / job_id, error);
            auto remove = prepare(
                store->db, "DELETE FROM video_upload_parts WHERE job_id=?;");
            if (remove) {
                sqlite3_bind_text(remove.get(), 1, job_id, -1,
                                  SQLITE_TRANSIENT);
                (void)sqlite3_step(remove.get());
            }
            if (!load_status_locked(store, job_id, out))
