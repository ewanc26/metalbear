...
bool uvarint(const uint8_t *buf, size_t limit, size_t &i, uint64_t &x,
             std::string &err) {
    x = 0;
    unsigned shift = 0;
    while (true) {
...
class CBORDecoder {
  public:
    CBORDecoder(const uint8_t *p, size_t n, std::string &err)
        : p_(p), n_(n), err_(err) {}
  ...
  private:
    ...
    bool aux(int ai, uint64_t &out) {
        ...
        switch (ai) {
            ...
            case 25:
                return read_be(2, out);
            case 26:
                return read_be(4, out);
            case 27:
                return read_be(8, out);
            default:
                return fail("reserved additional info");
        }
    }
    ...
    bool item(CBORValue &v) {
        ...
        switch (major) {
            case 0: {
                ...
                if (a > static_cast<uint64_t>(INT64_MAX))
            ...
            case 3: {
                if (ai == 31)
                    return fail("indefinite-length strings not supported");
                uint64_t len;
                if (!aux(ai, len)) return false;
                if (i_ + len > n_) return fail("truncated string");
                v = CBORValue::str(major == 2 ? CBORValue::T::Bytes
                                              : CBORValue::T::Text,
                                   reinterpret_cast<const char *>(p_ + i_),
            ...
            case 4: {
                if (ai == 31)
                    return fail("indefinite-length arrays not supported");
                uint64_t len;
                if (!aux(ai, len)) return false;
            ...
            case 5: {
                if (ai == 31)
                    return fail("indefinite-length maps not supported");
                uint64_t len;
                if (!aux(ai, len)) return false;
...
void cbor_enc_head(std::string &out, int major, uint64_t val) {
    if (val < 24) {
        ...
        for (int s = 24; s >= 0; s -= 8)
            out.push_back(static_cast<char>(val >> s));
    } else {
    ...
    switch (v.t) {
        ...
        case CBORValue::T::Simple:
            if (v.is_null) {
                ...
                out.push_back(v.boolv ? static_cast<char>(0xf5)
...
const CBORValue *cbor_map_get(const CBORValue &m, const std::string &key) {
    ...
    for (const auto &pr : m.map) {
        if (pr.first.t == CBORValue::T::Text && pr.first.bytes == key)
            return &pr.second;
    }
    return nullptr;
}
...
bool parse_car(const std::string &data, ParsedCar &out, std::string &err) {
    ...
    while (i < n) {
        ...
        blocks[std::string(reinterpret_cast<const char *>(p + start),
                           j - start)] =
    ...
    bool value(Json &out) {
        ...
        switch (c) {
            ...
            case '"':
                return string(out);
            case 't':
                return literal(out, "true", Json::T::Bool, true);
            case 'f':
                return literal(out, "false", Json::T::Bool, false);
            case 'n':
                return literal(out, "null", Json::T::Null, false);
            default:
                return number(out);
        }
    }
...
bool resolve_did_doc(const std::string &did, const std::string &plc_url,
                     std::string &source, Json &doc, std::string &err) {
    std::string url;
    if (did.rfind("did:plc:", 0) == 0) {
        ...
        size_t start = 0;
        for (size_t pos = rest.find(':'); pos != std::string::npos;
             pos = rest.find(':', start)) {
...
EVP_PKEY *load_secp256k1_pub(const uint8_t *pt, size_t len) {
    ...
    if (OSSL_PARAM_BLD_push_utf8_string(bld, OSSL_PKEY_PARAM_GROUP_NAME,
                                        "secp256k1", 0) != 1)
        goto out;
    if (OSSL_PARAM_BLD_push_octet_string(bld, OSSL_PKEY_PARAM_PUB_KEY, pt,
                                         len) != 1)
        goto out;
    params = OSSL_PARAM_BLD_to_param(bld);
    if (!params) goto out;
    if (EVP_PKEY_fromdata(cctx, &pkey, EVP_PKEY_PUBLIC_KEY, params) <= 0)
        pkey = nullptr;
out:
...
bool verify_signature(EVP_PKEY *pkey, const std::string &data,
    ...
    if (EVP_DigestVerifyInit(ctx, nullptr, EVP_sha256(), nullptr, pkey) == 1 &&
        ...
        ok = EVP_DigestVerifyFinal(
                 ctx, reinterpret_cast<const unsigned char *>(der.data()),
                 der.size()) == 1;
    }
    EVP_MD_CTX_free(ctx);
    return ok;
...
bool sha256(const std::string &in, uint8_t out[32]) {
    unsigned int len = 0;
    return EVP_Digest(reinterpret_cast<const unsigned char *>(in.data()),
                      in.size(), out, &len, EVP_sha256(), nullptr) == 1 &&
           len == 32;
}
...
int main(int argc, char *argv[]) {
    ...
    if (!roots || roots->t != CBORValue::T::Array)
        return fail("CAR header has no 'roots'");
    ...
    if (root.t == CBORValue::T::Tag) {
        if (root.tagvals.empty() || root.tagvals[0].t != CBORValue::T::Bytes)
    ...
    if (!root_bytes.empty() &&
        static_cast<unsigned char>(root_bytes[0]) == 0x00) {
        root_bytes.erase(0, 1);
    }

    std::cout << "CAR roots: " << roots->array.size()
              << " blocks: " << car.blocks.size() << "\n";
    std::cout << "root commit CID: " << "b" + base32_lower_nopad(root_bytes)
              << "\n";

    auto block_it = car.blocks.find(root_bytes);
    if (block_it == car.blocks.end())
        return fail("root commit block not present in CAR");

    // The root's CID must actually be the hash of its block (content
    // addressing).
    uint8_t digest[32];
    if (!sha256(block_it->second, digest)) return fail("sha256 failed");
    if (root_bytes.size() < 32 ||
        std::memcmp(root_bytes.data() + root_bytes.size() - 32, digest, 32) !=
            0)
    ...
    if (didv->t != CBORValue::T::Text)
        return fail("commit 'did' is not a string");
    if (version->t != CBORValue::T::Int)
        return fail("commit 'version' is not an integer");
    if (rev->t != CBORValue::T::Text)
        return fail("commit 'rev' is not a string");
    ...
    if (commit_did != did)
        return fail("commit claims " + commit_did + ", expected " + did);
    ...
    std::string sig;
    {
        ...
        if (sig_it->second.t != CBORValue::T::Bytes)
            return fail("commit 'sig' is not a byte string");
    ...
    if (!vms || vms->t != Json::T::Arr)
        return fail("key error: 'verificationMethod'");
    ...
    if (!mbj || mbj->t != Json::T::Str)
        return fail("key error: 'publicKeyMultibase'");
    const std::string mb = mbj->s;
    if (mb.empty() || mb[0] != 'z')
        return fail("publicKeyMultibase must start with 'z'");
    ...
    if (sig.size() != 64) {
        std::cout
            << "COMMIT SIGNATURE: INVALID - a relay would reject this repo\n";
        return 1;
    }
    EVP_PKEY *pkey = load_secp256k1_pub(
        reinterpret_cast<const uint8_t *>(pub_compressed.data()),
        pub_compressed.size());
    if (!pkey) return fail("failed to build public key");
    const bool valid = verify_signature(
        pkey, unsigned_cbor, reinterpret_cast<const uint8_t *>(sig.data()));
    EVP_PKEY_free(pkey);
    if (!valid) {
        std::cout
            << "COMMIT SIGNATURE: INVALID - a relay would reject this repo\n";
